...
// ---------------------------------------------------------------------------
// 断线 / 宽限期相关用例见 server/grace_test.go
// （t67：WS 断开不再立即丢弃房间与席位，也不再立即给对端发 peerLeft）
// ---------------------------------------------------------------------------
...
// stringsCount 统计 s 中 substr 出现次数（小工具，供宽限期用例断言"只发一次"）。
func stringsCount(s, substr string) int {
	if substr == "" {
		return 0
	}
	return len(strings.Split(s, substr)) - 1
}
