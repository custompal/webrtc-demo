...
> 3d. **B 备份的可见性差异（待对方补证）**：`tmp/jn-fix/pre-routeA-libwebrtc-java.jar`（1 206 602 B / 18:32:24 = B 备份）在**本容器内不存在**（`ls` 报 No such file）；native-dev 报其宿主侧为 **`0600 root` 不可读**（K-17 读侧又一实例）。两者不冲突（时点/视角差异），但**我按"本容器不存在"记录**，并请对方附 `ls -l` 原文 + 取数时刻再定；其内容与现值同 sha，可由交付件复算。
