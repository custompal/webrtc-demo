...
- `与 AAR 内 `classes.jar` 同 sha256` → **uniq = 2（`:545` 与 `:563`）**，不能当唯一锚点；改用**行内更长串**或直接按行号+上下文定位。
...
`d98939bb…`（t16/t23 前，major 61 重编件；AAR 内 `classes.jar` 同哈希）→ `7dbe8400…`（10:53:11，**磁盘上已无保留该哈希的文件**——我有界搜索 jar/aar 无命中；其"与 live 等价 508/508"只能依据当时的逐成员对比）→ `dc5f8919…`（11:05:10，508 条目，**无** `J/N`）→ **`0c776934…`（18:17:28，509 条目，含 `J/N` = handoff B，即本次落位）**。
...
#### (b) ⚠️ 材料指纹不符（请 native-dev 自订）
本轮消息给 `webrtc-build/t30/evidence/prefix-baseline.md` 为 **2 162 B / `0f581023…2536`**；**磁盘实测 = 5 205 B / `755c010938c52c70a1b66ad87324a7d5d37b1114bedb00c185402032168d7cd5`**（同目录 `README.md` 1 599 B、`three-source-crosscheck.md` 1 392 B，亦非该值）⇒ 引用的**大小/哈希已过期**。其**内容结论**我已独立复跑并确认（上表 + §13.9），故**不影响判据**，但引用时应以磁盘现值为准。
