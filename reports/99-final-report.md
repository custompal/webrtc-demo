...
APK            = app/build/outputs/apk/debug/app-debug.apk
                 ...
                 **证据等级（四层，按 webrtc-builder 建议写足）**：① **脚本机制**（受版控：事件前最后提交 `713ecd70:scripts/build_app.sh:485,488` / 现值 `aafd4e48…:563–570`，root 收尾 `chown -R 1000:1000` 含 `$APP/build`）；② **captain 自述**（`367adf2`）；③ **属主计数窗口**（19:06:40 非-1000 = 1047 → 19:16:17 = 0）；④ **`auth.log` 会话**：`2026-09-14T19:07:05.555820 systemd-logind[900]: New session 2380 of user root`（**距 `ctime` 24 s**）。**但无命令级留痕**（`/root/.bash_history` 3 908 B / mtime 10:26，无 19:07 条目；`/var/log/audit` 不存在）⇒ "chown 由该会话执行"属**强佐证、非逐命令证据**；**结论不变**（该件 19:06:23 前已回归、非第二次放置）。⚠️ 引用时勿取 19:08/19:09 那批**无日期**的 `Accepted publickey` 行（可能属 09-13）；④ 我**容器内不可复算**（`/var/log/auth.log`、`/var/log/audit`、`/root/.bash_history` 在容器内均不存在）。
