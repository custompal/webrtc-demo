...
| 真机缺陷修复（t23–t26，**18:33 后状态**） | **类存在性 ✅ / 16 KB 对齐 ✅ / 可诊断化 ✅ / 单测 42/0 ✅ / 可绑定性（jar/AAR）✅** —— 18:17:28 已落位 **B**（`J/N.class` `1ff8d3ff…` + `GEN_JNI.class` `a6e7edcf…`、`GEN_JNI` `native=0`、193↔193、194/194，§13.22）。**（历史时点陈述）APK 侧当时仍待 t33**：现行 `721df1c8…`（33 293 061 B / 11:28:34）由**未落位 jar** 构建、dex 内无 `LJ/N;` ⇒ **标注为"已被取代的历史轮次交付物"**（18:39 起 t33 构建窗口内该文件已从仓库清理；历史证据取仓外快照 `artifacts/app-debug-721df1c8.apk`，同哈希）⇒ **现状（本行已闭合）**：K-15 = **已闭合**，APK 侧锚点 = **`30c41ac9…`**（dex `LJ/N;` = 3），verifier 按 captain 放行口径复验 **verdict = pass**（§13.24/§13.25/§13.26） |
...
| **K-15** | ~~高（未闭合）~~ **已闭合（jar/AAR ✅ + APK ✅；原"APK 待 t33"分层已完成）** | **原缺口（已修复）**：jar 内 `GEN_JNI` 曾为 Placeholder（194 可读名 `static native`、0 转发），`.so` 为 hashing/short-proxy（193 `Java_J_N_<hash>`、无注册表）⇒ 两条绑定路径均不成立。**现行（18:17:28 落位、18:33:42 复核）**：交付 jar/AAR 为 **B 形态** —— `J/N.class` = `1ff8d3ff…`、`GEN_JNI.class` = `a6e7edcf…`、`GEN_JNI` `static native` = **0**、`J.N` native **193**、`jni_mangle` 双向差集 **0/0**、方法数 **194/194** ⇒ **jar/AAR 侧可绑定成立** | **APK 侧已闭合**：交付锚点 **`30c41ac9…`**（33 309 445 B；构建日志 `6f02e949…` 记"本次产出 APK = `30c41ac9…`"），dex 内 `LJ/N;` = **3**（原 `721df1c8…` 为 **0**，已标"历史轮次"）。**verifier 独立复验见 §13.24/§13.25/§13.26（verdict=pass，含三值门禁 + §13.3 四条 + 四 `.so` 护栏 + N-1..N-5）**；仍未验证项仅剩**真机运行期**（首调/注册/Camera2/首帧）。**独立第二方复验 = `reports/99-t34-appendix.md`（184 行 / `36bad740d32979bf…`，verdict = **pass**）** —— 本报告 §13.25 对其逐项复算、§13.26 为 captain 放行口径下的单批复验。 |
...
| P-14 | **跨侧计量"尺子不同"造成的假红/假矛盾**（verifier 20:04–20:10 实测） | ① dex 侧 AV1 桩终止指令是 `throw`（`dexdump` 指令列末为 `throw v0`），class 文件侧才是 `athrow` ⇒ 在 dex 上 `grep -c athrow` = **0**（假红）；② dex 方法表含 `<init>` ⇒ `J/N`/`GEN_JNI` dex 方法数 = **195 = 194 `public static` + 1 构造器**，与 jar 侧 194 **不矛盾**；不区分 `Static fields`/`Direct methods` 段会把字段计成方法（`PCFJni` 得 **26** 假数，真值 25 方法 + 1 字段）；③ 容器 `/tmp` = **256 MB tmpfs** ⇒ 大 dex（44.7 MB）与 `dexdump -d`（7.43 M 行）必须在工作区做，空间不足时报 `Size is too small`/`ReadFileToString failed`，**形似"dex 损坏"实为磁盘满**；④ `cmp -s` 对**缺失文件**也返回非 0 ⇒ 逐件比对**先断言存在**（我据此一度误报"变体四 `.so` 不同"，重测四件全同） | **断言必须写清"哪一侧、哪把尺子"并附命令原文**；跨侧数字先做一次口径对齐再比较；大件 IO 落工作区；`cmp` 前先做存在性检查（详见 §13.25(c)(d)）；⑤ **"取错对象"型假不符**：android-dev 首版用 `GET /`（返回 **1528 B 的 HTML 索引页**）当 APK 比对 ⇒ 不符，改取 `/app-debug.apk` 后逐字节一致 —— 与 `\w` 丢 `$`、`awk` 假数、`dexdump -d` 全 0、`git check-ignore` 不证存在**同族**：**先验证"取的到底是什么"，再比数字** |
| P-15 | **回放文本（replay）的分诊与处置**：同一事实被反复以"过期读数"重发，导致成员间空转（本次会话：APK 锚点 `30c41ac9…` 被同源重发 **≥5 次**、`GEN_JNI` dex 归属 **3 次**、sidecar 时间戳 **2 次**） | 起因 = **D-13**（过期任务文本被调度回放）+ 各成员 inbox 交叉在飞；现象 = 每条重发都带一个**已被取代的时点读数**（如"现行 APK 仍是 `721df1c8…`"、"`GEN_JNI;` = classes.dex 2"） | **处置协议（webrtc-builder 提议 + 我采纳，双方各自执行）**：① 收到消息先判**时点**：若其引用的状态早于最近一次**已核**状态变更，则**只用一条新鲜读盘行回复**（`[读盘 HH:MM:SS] <对象> <sha256 前16>`），不重开论证；② 只在"**新**的实测差异"上进行实质讨论，且必须给"**文件 + 哈希 + mtime**"；③ 冻结期内的历史句一律**另开附加件**或在原地划删并指向更正段（P-13 ③④）。⇒ 目的：把"回放"从**争辩**降级为**读盘校验**（详见 §13.26(6) 的实例） |
| P-16 | **交付产物必须"落盘即快照到仓外"**（否则整包会随构建窗口消失，事后无法对实物复核） | 18:41:59 的 t33 产物 `30c41ac9…` 在 **19:02–19:04 第二次构建的 clean 窗口内一度从仓库路径消失**（webrtc-builder `[19:03:51]` / native-dev `[19:03:49]` 均读到 `No such file or directory`；`app/build/intermediates/dex/**` 正在写入），**若当时无快照，其读数将只存在于消息里**。**实测结果**：`artifacts/app-debug-30c41ac9.apk`（33 309 445 B / mtime **18:41:59**）**存在**且与仓内实体（19:05:19 拷回）**逐字节相同**（我 `[读盘 20:38:53]` `cmp`）⇒ 该次有惊无险 | **规则**：每轮产物落盘后立即 `cp …/app-debug.apk artifacts/app-debug-$(sha256sum … \| cut -c1-8).apk`（现 `artifacts/` 已有 3 份：`721df1c8…`（历史）、`30c41ac9…`（交付）、`ef29e00c…`（非交付变体））；**判据一律落在"不随构建漂移的对象"上**（快照或六载荷），**不要**把"仓库路径存在性"当证据（P-13/P-15 同族）。**补充（webrtc-builder 建议 + captain 重要更正，已采纳）**：**测试原始输出与 XML 摘要必须固化到"被 git 跟踪的路径"** —— ⚠️ `.gitignore:83 = /reports/logs/*` 会把 `reports/logs/**` **整目录忽略**（我实测：该目录**盘上 45 个文件、`git ls-files` 跟踪 0 个**）⇒ 正确落点是 **`reports/10-*.log`**（如已成库的 `reports/10-t33-testDebugUnitTest-20260914-190455.log`、`reports/10-t33-captain-testDebugUnitTest-20260914-184232.log`）；`app/build/test-results/**` 属易失目录（19:02 的 clean 曾整目录清空、19:07:05 因重跑重建）⇒ **不得当长期证据**。 |
| P-17 | **"固化"必须是"入库"** —— 证据落在被忽略目录里等于没固化 | 我实测：`reports/logs/**` **盘上 45 个文件 / `git ls-files` 跟踪 0 个**（`.gitignore:83` = `/reports/logs/*`，且其上方注释说明"`!reports/**` 会重新纳入、故必须用内容级 `*` 模式"）⇒ **本报告对 `reports/logs/**` 的多处引用属"盘上存在、未入库"**（如 `reports/logs/final-nocache-assembleDebug-20260913-214035.log`、`reports/logs/final2-testDebugUnitTest-20260914-114748.log`、`reports/logs/t26d-testDebugUnitTest-20260914-112836.log`） | **规则**：新证据一律落**被跟踪**路径 `reports/<name>-<时间戳>.log`；引用 `reports/logs/**` 时**必须标注"未入库、仅本机盘上"**（captain 在 t34 指令中点明该细节；`reports/10-t33-*.log` 为已入库等价件） |
...
- **读侧终核（我 `[读盘 21:02:53–21:03:03]` 全树实测，采纳 native-dev 的谓词更正）**：**唯一真判据是 `! -readable`，不是 `-perm 600`、也不是 `-user root`**。实测：`gen` 路径下不可读文件 = **0**；`src` 全树（65 692 个目录）不可读**目录 = 8**（`third_party/instrumented_libs/binaries/msan-{chained,no}-origins-noble-lib/{ld,lib,sources}` ×6 + `build/linux/debian_bullseye_{amd64,i386}-sysroot/debian` ×2，均 `root:root 750`）、不可读**文件 = 15**（`third_party/test_fonts/**` 13 个字体 + `third_party/android_sdk/public/.cipd/pkgs/{0,1}/.lock` 2 个）——**全部不在任何构建/交付路径上**。⇒ 结论："**`gen`/`out`/`app/build` 的读侧影响 = 0**；`src` 全树另有 8 目录 / 15 文件对非 root 不可读，属无关资产"。**另采纳其建议：不对构建树做 `chmod`**（该件非交付产物，冻结窗口内写入需同流程协调，收益 0、风险非 0）。
  - **`-perm 600` 两个口径的定稿计数（我 `[读盘 21:14:21]` 自跑复算，与 native-dev 同值）**：`gen` 总文件 **2 793**、**`-perm 600` = 505**、其中 **`-perm 600 -user root` = 0**、**`! -readable` = 0**；`out` 全树 **`-perm 600` = 656**、**`-user root` = 0**、**`! -uid 1000` = 0**。⇒ "**505 / 656 是"任意属主的 mode-600 计数"**，**不等于 root 所有、也不等于不可读**"；native-dev 已撤回旧值 **"506"**（其 `reports/07`（791 行 / `ed00dfd4…`）内 `0600`/`不可读`/`505`/`506`/`K-17` 命中**全为 0** ⇒ 报告侧无需更正）。**宿主侧同一 inode 的属主对照 = 未核**（容器不可见 `/opt/dsh-workspaces`、native-dev 亦无 SSH 通道）——如需闭环须由宿主侧观测者给原样输出 + 时刻。 我无法在其不可读期间核对 sha256、也无法比较内容；"大小相同"当时只是**弱证据**；如需我核，请 `chmod 644`（或 `chown node:node`）该文件，我一条命令即可复核（`sha256sum` + 解包比对两份 `.java`）。
...
> 3d. **B 备份（已改名；现已可读并复核闭环）**：原 `tmp/jn-fix/pre-routeA-libwebrtc-java.jar` **该名已不存在**；实际件 = **`tmp/jn-fix/backup-B-before-A-incident-20260914-183224.jar`**。我 `[读盘 21:20:40]` 实测：**`root:root 644`（uid 1000 可读）**、1 206 602 B、mtime `18:32:24.288`、sha = **`0c776934c1452b7b…`**、**509 条目**、`J/N.class 1ff8d3ff…`(6 924 B) + `GEN_JNI.class a6e7edcf…`(24 910 B) ⇒ **内容即"事故前 B 备份"**。该件**曾一度为 `root:600` 不可读**（当时"在盘但不可读"的说法成立），**后由持写权者 `chmod 644` ⇒ 旧表述作废**。基座件另为 `tmp/t31/pre-routeA-libwebrtc-java.jar` = **`dc5f8919…`**（1 187 970 B / 11:05:10）与 `tmp/t31/pre-routeA-libwebrtc-arm64.aar` = **`e066e456…`**（6 489 244 B）—— **两者不同物**，勿混。
...
- **断言集"规范六数"（native-dev 定稿 + 我按原始命令逐条复跑）** —— 关键纪律：**两侧过滤器必须分开**（`J.N` 用 `static native`，`GEN_JNI` 用 `^  public static`）；混用会静默得 **0** 或 **195**（下附负对照）。
  ...
  **t34 用法**：这六数在 v2 门禁（`t34-gate-post-landing.md`）中为**基线断言**；t34 在新 APK 的 **dex 侧**与 jar 侧各跑一次，断言"jar 侧六数不变 **+ dex 内 `LJ/N;` 存在**"。
  **⚠️⚠️ 本行原写"dex 内 `LJ/N;` = **1**（v2 期望值）"，是**我自己的错**（`webrtc-builder` 19:02 提醒后我复核确认，见 F-7）**：那是**两把尺子**被混写 ——
  | 口径 | 命令 | 期望值 |
  |---|---|---|
  | **jar 条目前缀**（v2 差异点①）| `jar tf <jar> \| grep -c '^J/'` | **1**（仅 `J/N.class`）|
  | **APK dex 内串**（本轮实测）| 逐 dex `grep -a -o 'LJ/N;'` | **3**（`classes.dex` 2 + `classes13.dex` 1；历史件为 0）|
  ⇒ **断言写法**：jar 侧用 `^J/` **= 1**；dex 侧用 **"存在 / ≥ 1"**（实测 3，跨 2 个 dex）——**不得写成 dex `== 1`**（会假红）。
  ...
  **为什么在 `GEN_JNI` 上两者恰好相等（解释，非巧合于签名）**：`GEN_JNI` 的 **194 条方法全为 `public static` 且方法名均以 `org_` 开头**，而 `javap` 每个方法名只出现一次 ⇒ `grep -c 'org_'` 与 `grep -cE '^  public static '` 同值。**注意**：`org.`（点号）与 `org_`（下划线）**不同字符**，`grep 'org_'` 不会匹配 `org.jni_zero` 这类**签名/类名**——故"把签名里的 `org.` 也数进去"这一解释**不成立**（我实测 `GEN_JNI` 中 `org_` 命中行 = 194 = 方法数）。
...
>
> **第三方时点旁证（webrtc-builder 只读实测，补入本节）**：`[19:02:39]` 路径存在 = `30c41ac9…`（33 309 445 B）；**`[19:03:51]` 同一路径 `No such file or directory`**（`app/build` 被 clean）⇒ 证实 **19:02–19:04 期间确有第二次构建在跑**（其日志即 `10-t33-nocache-assembleDebug-20260914-190227.log`，产出 `ef29e00c…`）。**现盘（我 `[读盘 20:38:16]`）**：路径存在、mtime **19:05:19**、sha `30c41ac9…`、与仓外快照 `artifacts/app-debug-30c41ac9.apk` **逐字节相同** ⇒ **"19:03:51 缺失"是构建窗口内的瞬时态，非终态**（该窗口 `output-metadata.json` 因属第二次构建而保留 `19:04:43`，见 F-5）。
>
> **⚠️ 反例登记（F-8）：不得把"19:05:19 的 `cp` 拷回"读成"第二次构建"** —— native-dev 报"**两次构建同 sha `30c41ac9…` ⇒ 打包可复现**"，**与盘上证据相反**。四对象实测（我 `[读盘 20:40:07]`）：
> | 对象 | sha256 | size | mtime | 性质 |
> |---|---|---|---|---|
> | 第一次构建日志（`…183903.log`）| **`30c41ac9…`** | 33 309 445 B | 18:41:59 | **构建产物 #1** |
> | 第二次构建日志（`…190227.log:80`）| **`ef29e00c…`** | 33 309 445 B | 19:04:43.919 | **构建产物 #2** |
...
>
> **captain 裁定落文（2026-09-14，我按其要求写死并核对归档件）**：**交付 = `30c41ac9…`**；**`ef29e00c…` = 等价产物、非交付**（来自 **19:02:27 未获授权的重复构建**；归档 README 记执行者为 env-installer）—— 与交付件**仅 7 个次级 dex 分片不同**（`classes3/5/6/9/11/12/14`），而 **`classes.dex`（含 `J/N`）、`classes13.dex`（含 `GEN_JNI`）、全部 `.so`、`AndroidManifest.xml`、`resources.arsc` 逐字节相同** ⇒ **语义等价、sha 身份不同**。归档位置（我可读核对）：`/data/dsh/home/workspace/tmp/t38-unsanctioned-rebuild/app-debug-ef29e00c.apk` = `ef29e00c5217b5cd0c32f97d196800c8e7068f40ed5b9926a3f87b11636cc27d` / 33 309 445 B；`README.txt` = **1 209 B / `ab1a3cdba90d9e8bcb5990a4c759828be4b42ffde5fa953c1957a3f3729a0253`**。**⇒ 不得把 `ef29e00c…` 当交付物，也不得因两者不同而判 delivery 失败。**
> **可复现性数据点（captain 认可）**：同一输入/同一 jar+AAR（双钉 OK）下 `18:41:59 → 30c41ac9…`、`19:02:27 → ef29e00c…` ⇒ **APK 非逐字节可复现**——**但差异不在"分片/顺序"层**（正确表述，经 captain `reports/10 §9.14.6` 更正 + webrtc-builder 独立复算 + 我复核）：**逐 dex 类集合 14/14 完全相同（类数逐一一致）**，内容不同仅 7 个 dex（`classes3/5/6/9/11/12/14`；其中 **5 个同尺寸**，`classes3` **+4 B**、`classes14` **−8 B**），**JNI 关键条目（`classes.dex`/`classes13.dex`/四 `.so`/`AndroidManifest.xml`/`resources.arsc`）逐字节相同**，且整 APK 类描述符集合 **26 195 / 26 195**（双向差 0）。⇒ 正确口径 = **"语义可复现（类集合与逐 dex 分区一致）／整包 sha 不跨构建稳定"**；**不得把"19:05:19 标准路径 = `30c41ac9…`"当作"复现成功"**（那是拷回冻结件，见上 F-8）。
>
...
- ✅ **第三处同哈希已由宿主侧核对闭合**（captain 追加）：`/opt/apk-http/served/app-debug.apk` = **`30c41ac9…`**（33 309 445 B），与标准路径、`artifacts/` 留档 **`cmp` 逐字节相同**；且**公网直下（`GET /app-debug.apk`）与分片重组两条路径**均等于 `30c41ac9…`（android-dev 独立复核）⇒ 交付锚点共 **5 个可复算位点**同哈希。
  **宿主侧补充读数（webrtc-builder `[读盘 19:43:02]`，**容器不可复现**，我如实登记）**：served 件 **mtime `18:41:59.794`**（与 `artifacts/` 留档同刻 ⇒ 同一份 18:41 构建产物；仓内路径 19:05:19.310 系拷回未保时间戳，见 §13.26(1)）；`/opt/apk-http/parts/` = `part00..03`（8 388 608 ×3 + 8 143 621）**合计 33 309 445 B**（与锚点同尺寸），**`parts/SOURCE.sha256` = `30c41ac9…`**；systemd `ExecStart=/usr/bin/python3 /opt/apk-http/serve_apk.py … --apk /opt/apk-http/served/app-debug.apk`（t37 已改指冻结副本）。
  **宿主侧独立复验（**android-dev**，`[宿主 19:40:36–19:42:02]`，署名按其请求；我**不可复算**）**：① 三处交付面 `cmp` **逐字节相同**（served == 仓内标准路径 == `artifacts/app-debug-30c41ac9.apk`）；② `HTTP GET /app-debug.apk` **全量下载 = `30c41ac9…` / 33 309 445 B**（`Accept-Ranges: bytes`）；③ **分片路径**：`parts/` 4 片重组 sha = `30c41ac9…`、本机 `SHA256SUMS` 4/4 一致、`HTTP /app-debug.apk.part00 == 本地 part00`、HTTP 清单 == 本地清单；④ `SOURCE.sha256 = 30c41ac9…`、`apk-http` = active、`serve_apk.py` 进程 = 1 ⇒ **附录 §7.1 的"唯一静态未核项"由 android-dev 闭合**（§7.2 真机首调仍不可静态替代）。
- ⏳ **留待 t34 复算（本节未复核）**：① `.dex` **方法级**计数（`J.N` 195/193、`GEN_JNI` 195/0）需 dex 解析器或 `dexdump` 级工具；② `ef29e00c…` 与锚点的"**`code_items` 逐字节相同**"结论（我已有的旁证是：7 个差异 dex **长度仅差 4 B / 8 B、其余同长**、类计数一致 ⇒ 与"元数据/指针级差异"相容，但**不等于**已证"机器码逐字节相同"）。⇒ 这两项我在 **t34** 用可复算方式补齐或如实标"未复核"。
  ...
  - **✅ 两项均已由 t34 补齐（captain 追加，`reports/99-t34-appendix.md` = 184 行 / `36bad740…`，verdict = pass）**：① 方法级计数实测 `J.N`（classes.dex）**195/193**、`GEN_JNI`（classes13.dex）**195/0 且声明 AV1 桩**、`PCFJni`（classes14.dex）25/0/1；② **`code_items` 机器码逐字节相同**（区级 `map_list` + 方法级 `code_off` 两路独立），且根因已定量到键级（147 条 D8 `~~~{class→hash}` 不稳定令牌，全部为 Kotlin lambda 的 `*$$ExternalSyntheticLambda*`）。旁证冻结件：`webrtc-build/t39-apk-crosscheck/t39-apk-diff.freeze-682e02cad862cd76.log`（242 行 / `682e02cad862cd760b9326af1e46f934dfa48f93e87d00c7c4de76efbaee1ba2`）。**引用说明（native-dev 澄清 + 我复核）**：`t39-apk-diff.log` 是**单调追加**日志 —— 会话中先后出现的 75 行 / `28fa86c4…`、95 行 / `cf88a702…`、109 行 / `111f77d4…`、126/`b4dcf35f…`、143/`b2250a75…`、171/`23783dbb…`、194/`903b6dd8…`、203/`b7c7715c…`、213/`d0967d09…`、225/`0d7339e3…` 等读数是**该文件各时点的真实状态**（非覆盖写、非第二路径），最终停在 **242 行 / `682e02ca…`（19:31:18）** 并由 `cp -p` 冻成上述 `freeze-…` 件（两份 `cmp` 逐字节相同）⇒ **长期引用一律以冻结件为准**。
...
APK            = app/build/outputs/apk/debug/app-debug.apk
                 ...
                 **脚本级来源（webrtc-builder 记录级证据 + 我 `[读盘 21:34:52]` 复核）**：`scripts/build_app.sh` 现值 `aafd4e48f19889dcea4b3f809b8a529bc5b7883dfcf96190fdc664d237314f39`（36 294 B / mode 755 / mtime 19:43:29）**:563–570** 含 `if [ "$(id -u)" = "0" ]; then … chown -R 1000:1000 "$JNILIBS" "$LOG_DIR" …` 与 `for d in "$APP/build" "$APP/.cxx" "$PROJ/.gradle" "$PROJ/.kotlin"; do chown -R 1000:1000 "$d"`（**含 `app/build`**）；该块由 **`198d514`（t10）** 引入，且 **19:07:29 之前最后提交 `713ecd70…`（19:05:51）的 `:485,:488` 已含同一块** ⇒ **"属主归零"是脚本行为、可复现，不是假设**。
> **证据等级（四层，按 webrtc-builder 建议写足）**：① **脚本机制**（受版控：事件前最后提交 `713ecd70:scripts/build_app.sh:485,488` / 现值 `aafd4e48…:563–570`，root 收尾 `chown -R 1000:1000` 含 `$APP/build`）；② **captain 自述**（`367adf2`）；③ **属主计数窗口**（19:06:40 非-1000 = 1047 → 19:16:17 = 0）；④ **`auth.log` 会话级（收紧版，取代早前 19:07:05/session 2380 的说法）**：`19:07:28.969310 New session 2381 of user root`（`Accepted publickey 19:07:28.958380`）→ `19:07:29.197655 New session 2382 of user root`（`19:07:29.185520`）→ **交付件 `ctime = 19:07:29.818728`（session 2382 开后仅 0.62 s）** → `19:07:30.008401 su[669973]: (to admin) root on none`（su 会话 19:07:30.009980→19:07:30.082683）→ `19:07:30.096719 Removed session 2381`；`mtime 19:05:19.310` / `ino 3144249` / `uid 1000:1000` 全程不变 ⇒ 形态与"**root 会话先 `chown`、随后 `su admin`**"吻合（ctime 落在 session 2382 之后、`su` 之前）。**但无命令级/内核审计留痕**（`/root/.bash_history` 3 908 B / 末次写 10:26、无 19:07 条目；`/var/log/audit` 不存在；`auth.log` 只记会话不记命令）⇒ 定性为"**强佐证**"，**既不写"已定论"、也不写"未记录来源"**；**结论不变**（该件 19:06:23 前已回归、非第二次放置）。⚠️ **只引带 `2026-09-14T` 前缀的行**（2381/2382/su），无日期批次可能属 09-13。**我容器内不可复算**（`/var/log/auth.log`、`/var/log/audit`、`/root/.bash_history` 均不存在，`[读盘 21:35:58]`）⇒ 第④层按"宿主侧读数、署名 webrtc-builder"登记。
...
- **F-5（low，证据链）**：`output-metadata.json`（404 B / mtime **19:04:43.947**，属 `ef29e00c…` 那轮）与路径上 APK（19:05:19 拷回、内容 = 18:41:59 构建）**不同源** ⇒ 不得用 sidecar 给 APK 定年（详见 §13.26(1)）。
  **更强形式（webrtc-builder 提出 + 我 `[读盘 21:42:39]` 复核）**：该 sidecar **根本不含任何 `sha`/`size`/`checksum`/`hash` 字段**（我 `grep -ciE 'sha|size|checksum|hash'` = **0**；实际字段仅 `version`/`artifactType`/`applicationId`/`variantName`/`elements[{versionCode,versionName,outputFile}]`/`elementType`/`minSdkVersionForDexing`）⇒ 它连"轻量标识"都做不到，**只声明 variant 元数据**。⇒ **APK 的唯一标识 = `sha256 30c41ac9…` + 构建窗口钉值日志（`6f02e949…`）**。
...
- **F-8（medium，结论性误读）**：他方报"两次构建同 sha `30c41ac9…` ⇒ 打包可复现"——**与盘相反**（第二次构建日志 `…190227.log:80` 明记 `ef29e00c…`、mtime 19:04:43；`artifacts/app-debug-ef29e00c.apk` 与之同刻同哈希；仓内路径 19:05:19 是 `cp` 拷回、mtime 即拷贝时刻）。**正确结论 = APK 整包 byte-reproducibility = false**（详见 §13.23 反例登记）。**教训**：先钉"对象性质（build 产物 / copy / 快照）"再谈可复现性。
  **归属限定（native-dev 请求 + 我 `[读盘 21:48:05]` 复核）**：该错误说法**只存在于消息层**；被指两方的**落盘材料自始就是相反口径** —— `reports/99-t34-appendix.md` 明写"**APK 整包不可复现**：同输入、T0/T1 双钉均 OK，仍产出 `ef29e00c…` ≠ `30c41ac9…`"（§9）"仅作'整包不可复现'的实证"（§6），`t39-apk-diff.freeze-…log:181,225` 亦写 "**byte-reproducibility = false** 的第二实证" ⇒ **无需改动其文件**；本项仅纠正消息层表述。
- **F-9（low，判据精化）**："AV1 可读名 ⇒ B 形态"**不成立**（A jar 亦含该名 1 个 class、旧 APK `721df1c8…` 含 2 处）⇒ 它只证"调用点/绑定类已入包"；**B 排他标记 = 桩字面量 `Native method not present`**（A jar 0 / 旧 APK 0 / B jar 2 class / 交付 APK 2 处）⇒ **APK 层形态判据改为合取 `LJ/N;` > 0 ∧ `stub_msg` > 0**；该字面量在 `app/src` 仅见于单测（`app/src/test/.../JniBindingClasspathTest.kt:60`，`main` 0 处），不影响判据（详见 §13.25(h) 表格）。
  **三条限定（native-dev 提出 + 我复核采纳）**：① 该字面量源自**本 checkout 的 jni_zero 模板**（`_stub_for_missing_native`）⇒ **换模板即失效**，故它只是**事后旁证**，**不能替代**"构建窗口内 jar/AAR 钉值 + `.so` 载荷（`757cef81…`）+ **真机首调**"；② 单看 `av1_readable`（旧 APK 2 / 交付 3）**不可判 A/B**（A 的 jar 同样 48 个 `*Jni`，常量池照引可读名）；③ **条目数 / `*Jni` 计数亦不可判 A/B** —— 我实测 A jar（`c289b4df…`）与 B jar 同为 **509 条目 / `*Jni.class` 48 / `J/N.class` 1 / `GEN_JNI.class` 1**，差异只在 **`GEN_JNI` 方法数（194 vs 193）与 `J.N` 的 AV1 桩（有/无）**。
...
- **`app/build` 属主复核（两级更新，`[读盘 21:08:30]`）**：① `[20:40:57]` 曾实测 `app/build` = **`node:node 755`（mtime 19:07:02）**、非 node 项 = 0 ⇒ webrtc-builder 报的"`root:root` / root-owned 1047"**当时已被 19:07:02 的 uid-1000 构建取代**；② **但 20:50–20:52 captain 的宿主（root）单测重跑**曾产生 **657 个 `root:root` 项**（`intermediates` 400 / `tmp` 232 / `reports` 11 / `test-results` 9 / `generated` 4 / `outputs` 1；mtime 20:50:39–20:52:31）——**交付路径 `outputs/**` 仅 1 项且交付 APK/jar 前后逐位不变**（见 §13.26(4)）；**③ `[读盘 21:13:13]` 该 episode 已清理**：`app/build` = `node:node 755`（目录 ctime **21:11:01**）、**非 uid 1000 项 = 0**（总 1 343 项），20:52:31 的 5 份 XML 仍在且归 node（合计 46/0/0）。⇒ **口径**：整件事是**瞬时 episode、非遗留阻碍**（`app/build/**` 属构建目录且 gitignore），**操作性根治 = 构建/测试一律以 uid 1000 运行**（与 `.git/index` 被 root 重建同源）。
  **K-17 读侧最终口径（我 `[读盘 21:50:52]` 复测）**：**仓库内 `mode 600 ∧ uid≠1000` = 0**；且**全仓（含 `.git`）`find . ! -uid 1000` = 0、`! -user node` = 0** —— 先前记录的**残余 12 个 `root:root 644`（`app/.cxx/**`：11 个 `*.cpp.o` + 1 个 `configure_fingerprint.bin`，webrtc-builder 精化后的清单）现已不存在**（或已归 uid 1000、或该 CMake 暂存目录被清理/重建；`app/.cxx` 现 70 项、全 `node:node`）⇒ **仓库内属主/读侧残留归零**。
  **✅ 仓外 3 件原不可读者已于 20:53–20:54 全部解除并逐件复核（K-17 读侧就此闭合）** —— 三件的 **ctime 均为 `20:45:29`**（= 持写权者 `chmod 644` 事件；**`sha` 与 `mtime` 均未变 ⇒ 内容未被动过**，可按 native-dev 建议作"改动前后 `sha+mtime+ctime`"留痕）：`artifacts/pre-deploy-7dbe8400.jar`（`mtime 11:03:36`）、`tmp/jn-fix/backup-B-before-A-incident-20260914-183224.jar`（`mtime 18:32:24`）、`tmp/jn-fix/QUARANTINE-A/repro-c289b4df.jar`（`mtime 18:32:10`）。**作用域计数（我 `[读盘 21:27:00]` 复算）**：`webrtc-build/src/out` / `code/webrtc-demo` / `artifacts/` / `tmp/jn-fix/` 四处 **`-perm 600 ∧ uid≠1000` 均为 0** ⇒ 早前"全工作区 = 2 件"的读数**已被 20:45:29 的 chmod 取代**。：
  | 件 | 现权限 | sha256 | 我复核内容 |
  |---|---|---|---|
...
- **captain 四步复核（我 `[读盘 20:55:46]` 单批实测）**：① `sha256 = 30c41ac9…` / **33 309 445 B** ✅；② 逐 dex 出现次数 `LJ/N;` = **3**（2+1）、`GEN_JNI;` = **3**（0+2+1）、`PCFJni;` = **2**（`≥1` 口径）✅；③ 四 `.so` `LOAD p_align` 全 **0x4000**、`libjingle 757cef81…` 与 `libc++_shared c9dbf4ec…` 均与 `jniLibs` **逐字节相同** ✅；④ K-15 的 APK 侧 = **已闭合**（全文 12 处"K-15 已闭合"；残 3 处"待 t33"均在**显式历史/引用他方过期口径**语境：行 76、418、1581）。单测按令**直接引用** 19:07:05 的 5 份 XML = **46/0/0**（不重跑）。**（前次三步记录：**APK `30c41ac9…`（33 309 445 B）；`GEN_JNI` **195（含 `<init>`）/ 194 `public static` / `static native` 0 / 转发 193 / `athrow` 1**；dex 出现次数 `LJ/N;` = 2+1、`GEN_JNI;` = 0+2+1、`PCFJni;` = 2（**均 ≥1，未写 ==3**）；四 `.so` `p_align` 全 `0x4000`，`libjingle 757cef81…`/`libc++_shared c9dbf4ec…` 与 `jniLibs` **逐字节相同**。第 2 步：5 份 XML = **46/0/0**（19:07:05）；容器内复跑仍被 AGP 拒绝（`* What went wrong: 25.0.4.1`，APK/jar 前后不变）。第 3 步：§13.22/§6/§12.5 的 K-15 均已是"**已闭合**"、`721df1c8…` 为历史轮次、`GEN_JNI` 措辞按"194/0/193/`athrow` 1" ⇒ **verdict 维持 `pass`**。

#### (9) **口径三写**（captain 更正 + 我 A/B 两侧实测复核）：方法数 vs 出现次数
- **背景**：captain 指出其先前给的 `J.N=3 / GEN_JNI=3 / PCF_Jni=2` 是**引用/描述符出现次数**（跨 dex 会重复计入），**不是类定义数**；A/B 判别应用**类定义级 + 方法数**。**我按 A、B 两侧逐项复核，结论一致**：
  | 口径 | 命令（cwd 同前） | **B 落位件**（`1ff8d3ff…`/`a6e7edcf…`） | **A 变体**（`9ada0641…`/`8f3ce613…`） |
  |---|---|---|---|
  | class 文件**方法总行数（含构造器）** | `javap -p X.class ｜ grep -c '('` | `J/N` **195** / `GEN_JNI` **195** | **194** / **194** |
  | `public static` 方法数 | `javap -p X.class ｜ grep -cE '^  public static '` | **194** / **194** | **193** / **193** |
  | **反射 `getDeclaredMethods()`（不含构造器）** | —（口径说明） | **194** / **194** | **193** / **193** |
  | `static native` 声明数 | `javap -p X.class ｜ grep -c 'static native'` | **193** / **0** | **193** / **0** |
  | **类定义级 `dexdump` 方法数** | 逐 dex `dexdump` 类块计数 | **195** / **195** | **194** / **194** |
  | **AV1 声明** | `grep -c 'LibaomAv1Encoder_create'` | **1**（两侧各 1） | **0** |
  | **出现次数（引用，非定义）** | 逐 dex `grep -a -o` | `LJ/N;` **3**（2+1）、`GEN_JNI;` **3**（2+1）、`PCFJni;` **2** | 同类 3 / 3 / 2 |
- **断言规则（据此收口）**：
  1. **A/B 判别** ⇒ 用**类定义级 + 方法数 + AV1 声明**：B = **195/195 且声明 AV1**；**A = 194/194 且不声明 AV1**（可区分）。
  2. **引用/出现次数** ⇒ 只写 **`≥ 1`（或"至少一个 dex 含"）**，**不得写 `== 3`**（dex 分片布局随 minSdk/多 dex 切分变化，写死会假红 —— 与 F-7 同类根因）。
