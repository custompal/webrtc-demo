...
> **可复现性数据点（captain 认可）**：同一输入/同一 jar+AAR（双钉 OK）下 `18:41:59 → 30c41ac9…`、`19:02:27 → ef29e00c…` ⇒ **APK 非逐字节可复现**（差异为 D8 多 dex 分片非确定性）；**不得把"19:05:19 标准路径 = `30c41ac9…`"当作"复现成功"**（那是拷回冻结件，见上 F-8）。
>
