# 15 — 原生重编 libwebrtc Java SDK（t16/t17，**最终交付 = Java 17 / major 61**）
...
- 结论：**✅ 成功**。最终交付 jar 内 **453/453 个 class 均为 `major version 61`（Java 17）**，由 **javac 原生重新编译**产生（非改字节），native 产物零改动。
...
> **可复现性（实测）**：以 `--release 17` 重跑 `ninja -C out/Release-arm64 -j2 sdk/android:libwebrtc`（168 步 / 约 6 分钟）→ 产出**逐字节相同**的 jar（`d98939bb…`）✅
...
| **字节数** | **1 048 264 B**（t5 原 build 产物 1 051 957 B；t10 patch 版 555 728 B — **该 patch 版系 re-zip 产物，其字节已不可得**，见 §8 的 provenance 说明） |
| **sha256** | **`d98939bbf0c0cd071baff19004a4dc2f602997b70e33e0fc3669c5fccb5f6543`** |
| **class 版本** | **453/453 = major 61（Java 17）** |
| 生产端同源 | `out/Release-arm64/lib.java/sdk/android/libwebrtc.jar`（同哈希） |
| AAR 内 `classes.jar` | 已同步更新（见 §5），**三处 sha256 前 16 位均为 `d98939bbf0c0cd07`** |
| 备查 | `libwebrtc-java.jar.v55-java11`（**中途 Java 11 版本，已归档作废**）、`libwebrtc-java.jar.orig-jdk25`（t5 原始 v69） |

**AAR（重打包后）**：`third_party/libwebrtc/java/libwebrtc-arm64.aar` = 6 456 926 B，sha256 `c7907be25a95d9f861e550036f4ebc99be0a4590972ecc3298a477b14ebf8600`；其 **内部 `.so` 逐字节未变**（sha256 `757cef81…`，见证据 4）。
> 注：不能以文件大小判断 AAR/jar 版本（各版本大小接近）；**判据是内含 `classes.jar` 的 sha256**。

**产物哈希一览（最终）**：
| 产物 | sha256（前 16 位） | 大小 |
|---|---|---|
| `libwebrtc-java.jar`（交付） | `d98939bbf0c0cd07` | 1 048 264 |
| AAR 内 `classes.jar` | `d98939bbf0c0cd07` | 1 048 264 |
| `jni/arm64-v8a/libjingle_peerconnection_so.so` | `757cef8128bf9151` | 12 946 912 |
| `libvpx/lib/libvpx.a` | — | 1 929 142 |

### 证据 1：class 总数仍为 453，且**全部**为同一版本（无混合字节码）
```
$ unzip -oq libwebrtc-java.jar -d . && find . -name '*.class' | wc -l
453
...
| 对比 | 结果 |
|---|---|
| 现行 v61 jar vs t10 补丁 jar（v55，**历史/已退役**） | 453 个 class 全部内容不同（0 相同） |
...
| 文件 | 字节 | 含义 |
|---|---|---|
| `libwebrtc-java.jar` | 1 048 264 | **现行交付**（javac 原生 **Java 17**，sha256 `d98939bbf0c0cd07…`，453 class 全 **major 61**） |
| `libwebrtc-java.jar.v55-java11` | 1 048 264 | 中途 v55 的归档副本（**已作废**，非现行） |
| `libwebrtc-java.jar.v61-java17` | 1 048 264 | 首轮产出的原生 Java 17 版（major 61），备查（**非最终态**） |
...
| `libwebrtc-arm64.aar` | 6 456 926 | 现行 AAR，内含与 jar 同哈希的 `classes.jar`（`d98939bb…`） |
...
- 可复现性收益仍然成立：补丁固化后以 `--release 17` 重跑（168 步 / 约 6 分钟），产出**逐字节相同**的现行 jar（`d98939bb…`）✅。

### 8.4 给 verifier / 后续工作的检查清单（可直接复用；已按最终 v61 更新）
1. `unzip -oq <jar> -d X && find X -name '*.class' | while read c; do xxd -p -s6 -l2 $c; done | sort | uniq -c` → 应为 **453 × `003d`**（全 Java 17，无混合版本）；
2. `javap -v` 抽 3 个 class → `major version: 61`；
3. jar sha256 应为 **`d98939bbf0c0cd071baff19004a4dc2f602997b70e33e0fc3669c5fccb5f6543`**，1 048 264 B；且与 **AAR 内 `classes.jar` 同哈希**；
4. `.so` sha256 必须仍为 `757cef8128bf915109864ab92df29984dea17493dfe3417a73cd00fdc233259e`、12 946 912 B、stripped；`libvpx.a` = 1 929 142 B；
5. **Java 产物必须验证字节码目标版本**（这是 t5 的核验盲区：当时只验了"jar 内有 `org.webrtc` 类"，未验 major version）；
...
### 9.3 守卫规则（实测四个分支全部通过；目标值 `JAVA_RELEASE_TARGET=11`）
| 输入状态 | 期望行为 | 实测 |
|---|---|---|
...
- `build/android/gyp/compile_java.py:711` → **`'11'`**
- `build/android/gyp/turbine.py:110` → **`javac_cmd = ['--release', '11']`**
