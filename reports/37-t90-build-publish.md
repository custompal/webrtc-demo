...
**captain 2026-09-16 20:3x 追加要求已登记于此**：
1. **本次统一构建合并了 t87 + t89 + t91 前置改动**（即「编码自动降级兜底 + 视频流码率地板修复 + 质量降级 ScalingSettings(24,37)」），交付面为一份包、一个锚点。
2. **T1 口径**：captain 通知的 T1 = **20:16:05**；我在宿主机实测的 `app/src` 最后写入 = **20:15:38.476**（`encoder/Vp9VideoEncoder.kt`，sha256 `2fb3a425d92c9323…69e5be2`），**未观测到 20:16:05 的另一次写入**。两时刻**均早于本任务构建窗口（20:23:26–20:26:01）**，且窗口内 `app/src` 写入 = 0，故无论取哪个 T1 本锚点都覆盖该改动 —— 差异仅影响静默窗口的起算点，已在 §2 表首行并列登记。
3. **该改动确在包内（dex 旁证）**：已发布 `served` 本体 dex 实测 `ScalingSettings` = **9**、`Vp9VideoEncoder` = 18、`minStartBitrateBps` = 1、`bitrate_limit` = 1 ⇒ 质量降级策略与码率地板**两项都在**交付件中。
4. **t88 产物处置**：`reports/37-t88-build-publish.md` 顶部已按 captain 指令加「**已作废**」标注（未发布；served 从未被 t88 触碰；现役锚点为本任务 `0f60d901…fe45`）。

t90 的**契约正文未随派发送达**（我只收到 t88 的派发文本 + 任务牌上的标题「统一构建发布：合并打包 t87 + t89 并刷新公网快照」），故按既定构建口径执行：四段串行命令 + 计数闸门 + `publish_apk.sh` 发布 + 四方对账 + 公网复验，闸门取 **t87 基线（22 类/206 例）+ t89 新增（1 类/7 例 `Vp9BitrateLimitsTest`）= ≥23 类 / ≥213 例**（captain 后续确认口径一致）。
